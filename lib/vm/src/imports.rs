...
pub fn do_hello<A: BackendApi>(
    mut env: FunctionEnvMut<Environment<A>>,
    source_ptr: u32,
    destination_ptr: u32,
) -> VmResult<u32> {
    ...
    let source_data = read_region(&env, source_ptr, MAX_LENGTH_QUERY_REQUEST)?;
    ...
    let (result, gas_info) = env.data().api.hello(&source_string);
    process_gas_info::<A>(&env.data().clone(), &mut env, gas_info)?;
    match result {
        Ok(data) => {
            write_region(&env, destination_ptr, data.as_slice())?;
            Ok(0)
        }
...
pub fn do_debug<A: BackendApi>(
    ...
    if env.data().print_debug {
        let message_data = read_region(&env, message_ptr, MAX_LENGTH_DEBUG)?;
...
fn write_to_contract<A: BackendApi>(
    ...
    let result = data.call_function1(&mut env, "allocate", &[out_size.into()])?;
    let target_ptr = ref_to_u32(&result)?;
    if target_ptr == 0 {
        return Err(CommunicationError::zero_address().into());
    }
    write_region(&env, target_ptr, input)?;
    Ok(target_ptr)
}
...
//         let api = MockApi::default();
//         let (env, mut instance, store) = make_instance(api);
//         let api = MockApi::default();
//
//         let source_ptr = write_data(&mut store, &env, b"foo");
...
//         let api = MockApi::default();
//         let (env, mut _instance, store) = make_instance(api);
//
